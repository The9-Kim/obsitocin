...
```bash
# 초기화
obsitocin init --vault-dir ~/my-vault  # config + hooks + MCP 서버 등록
obsitocin init --llm-provider qwen     # LLM 제공자 지정

# 수집 파이프라인
obsitocin run                          # 수동 파이프라인 실행
obsitocin run --dry-run                # 미리보기
obsitocin run --llm-provider claude    # 제공자 오버라이드
obsitocin run --detect-pii             # PII 감지 활성화
obsitocin run --redact-pii             # PII 마스킹 후 저장
obsitocin run --skip-sensitive         # 민감 항목 건너뛰기

# 외부 소스 수집
obsitocin ingest https://example.com   # URL 수집 → vault에 저장
obsitocin ingest ./notes.md            # 로컬 파일 수집
obsitocin ingest <source> --project my-api --title "제목"

# 검색
obsitocin query "환불 정책"              # 하이브리드 검색 (BM25 + 벡터)
obsitocin query "환불 정책" --mode bm25  # 키워드 검색만
obsitocin query "환불 정책" --mode vector # 벡터 검색만
obsitocin query "환불" --type static     # 메모리 유형 필터
obsitocin query "환불" --category policy # 카테고리 필터
obsitocin query "환불" --importance-min 4 # 중요도 필터
obsitocin query "환불" --context         # 컨텍스트 모드 (LLM 주입용)
obsitocin concepts "venv"              # 주제 단위 집계 검색

# 위키 질의
obsitocin ask "환불 정책이 뭐야?"        # 위키 기반 Q&A (출처 포함)
obsitocin ask "Docker 설정" --project my-api  # 프로젝트 한정
obsitocin ask "Redis 캐시" --save       # 답변을 위키에 저장
...
obsitocin lint --min-knowledge 3       # 빈약 노트 기준 변경 (기본: 2)
