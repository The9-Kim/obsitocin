...
규칙:
- should_store: 업무 활동이나 배울 내용이 있으면 true. 작업 로그 용도로도 쓰이므로 넓게 판단해라.
  false 조건:
  · 인사, 단순 확인("네", "진행해줘"), 빈 응답
  · 시스템 내부 동작 보고 (배경 작업 ID, 세션 관리)
  · 에이전트/도구 자체의 운영 규칙 (예: delegate_task, load_skills, run_in_background)
  · 대화에 새로운 사실도 없고, 의미 있는 작업 수행도 없는 경우
  true 조건 (위 false에 해당하지 않으면):
  · 코드 작성/수정, PR 생성, 리뷰, 디버깅 등 실제 업무 수행
  · 도메인 지식, 업무 프로세스, 정책, 의사결정
  · 설정, 환경 구성, 문서 작성 등 프로젝트 관련 활동

- topics: should_store가 true이면 최소 1개 주제를 반드시 넣어라. 최대 3개 주제.
  - name: 백과사전/위키 문서 제목 수준. 기술, 도메인 지식, 정책, 프로세스 모두 가능.
    좋은 예: "Python 가상환경 (venv)", "결제 취소 정책", "코드 리뷰 프로세스", "인증 서버 아키텍처"
    나쁜 예: "요청 수락", "코드 실행", "데이터 관리" (대화 행위이거나 너무 막연함)
  - knowledge: 해당 주제에 대해 이 대화에서 배운/수행한 구체적 사실/방법/결정/규칙/작업 내용.
    요약하지 말고 fact를 나열. 해당 주제에 속하는 것만.
    좋은 예: "pip install -e . 로 개발 모드 설치 가능", "환불은 7일 이내만 가능", "AGENTS.md를 105줄에서 60줄로 압축함"
    나쁜 예: "가상환경에 대해 논의했습니다", "배경 작업 bg_xxx 완료"
  ...
  에이전트 내부 운영 메커니즘(delegate_task, load_skills 등)은 주제로 만들지 마라.
  단, 실제 수행한 업무 내용(PR 생성, 문서 정리, 설정 변경 등)은 해당 프로젝트 주제에 knowledge로 기록해라.
...
- work_summary: 작업 로그에 쓸 한 줄. "무엇을 했는지" 중심. should_store가 true이면 반드시 채워라. 비워두지 마라.
- tags: 영문 lowercase kebab-case. 3-5개.
- category: 가장 적합한 하나만.
- importance:
  1: (사실상 미사용 — should_store=false로 걸러짐)
  2: 단순 반복 작업 (커밋, 포맷팅, 파일 이동)
  3: 일반 업무 수행 (PR 생성, 문서 작성, 설정 변경, 코드 리뷰)
  4: 도메인/기술 지식 학습, 아키텍처 결정, 정책 수립
  5: 핵심 설계 원칙, 보안 정책, 불변 규칙"""
...
규칙:
- should_store: 업무 활동이나 배울 내용이 있으면 true. 넓게 판단해라.
  false 조건: 단순 알림, 빈 내용, 실질적 정보 없는 메시지.
- topics: should_store가 true이면 최소 1개 주제를 반드시 넣어라. 최대 3개.
  - name: 백과사전/위키 수준의 재사용 가능한 주제명
  - knowledge: 해당 주제에 대한 구체적 사실/정보/수행 내용
- work_summary: 이 콘텐츠가 무엇에 관한 것인지 한 줄. should_store가 true이면 반드시 채워라.
- tags: 영문 lowercase kebab-case. 3-5개.
- category: 가장 적합한 하나만.
- importance:
  2: 단순 반복 작업 (커밋, 포맷팅, 파일 이동)
  3: 일반 업무 수행 (PR 생성, 문서 작성, 설정 변경)
  4: 도메인/기술 지식 학습, 아키텍처 결정, 정책 수립
  5: 핵심 설계 원칙, 보안 정책, 불변 규칙
